...
#endif
            {
                if (!pressing && Mouse.current.leftButton.wasPressedThisFrame)   //When pressed (left click / touch can be acquired).    //押したとき（左クリック/タッチが取得できる）
                {
                ...
                {
                    if (Mouse.current.leftButton.isPressed)    //Continue pressing (* This function can not distinguish which finger when touching two or more)    //押下継続（※この関数は２つ以上タッチの場合、どの指か判別できない）
                    {
