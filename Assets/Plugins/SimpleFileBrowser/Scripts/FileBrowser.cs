...
#region Properties
    ...
    set {
      ...
      if (m_currentPath != value) {
        ...
        if (currentPathIndex == -1 ||
            pathsFollowed[currentPathIndex] != m_currentPath) {
          currentPathIndex++;
          if (currentPathIndex < pathsFollowed.Count) {
            pathsFollowed[currentPathIndex] = value;
            for (int i = pathsFollowed.Count - 1; i >= currentPathIndex + 1;
                 i--)
  ...
  public int SelectedFilePosition {
    get { return m_selectedFilePosition; }
  }
...
#region Interface Methods
  ...
  public ListItem CreateItem() {
    FileBrowserItem item =
        (FileBrowserItem)Instantiate(itemPrefab, filesContainer, false);
  ...
  public void SetItemContent(ListItem item) {
    ...
    if (isDirectory)
      icon = folderIcon;
    else if (!filetypeToIcon.TryGetValue(fileInfo.Extension.ToLowerInvariant(),
                                         out icon))
      icon = defaultIcon;

    file.SetFile(icon, fileInfo.Name, isDirectory);
    file.SetHidden((fileInfo.Attributes & FileAttributes.Hidden) ==
      ...
      if (drivesList != null && drivesList.Length > 0) {
        ...
        for (int i = 0; i < drives.Length; i++) {
          try {
            ...
            //	try
            //	{
            //		driveName = Directory.GetParent( drives[i] ).Name + "/" +
            //driveName;
            //	}
            //	catch
            //	{
  ...
  public void OnSubmitButtonClicked() {
    ...
    if (File.Exists(path)) {
      ...
      if (m_acceptNonExistingFilename) {
        if (!m_folderSelectMode &&
            filters[filtersDropdown.value].defaultExtension != null)
          path = Path.ChangeExtension(
              path, filters[filtersDropdown.value].defaultExtension);
...
#region Other Events
  ...
  public void OnPathChanged(string newPath) { CurrentPath = newPath; }

  public void OnSearchStringChanged(string newSearchString) {
    SearchString = newSearchString;
  }

  public void OnFilterChanged() { RefreshFiles(false); }

  public void OnShowHiddenFilesToggleChanged() { RefreshFiles(false); }
  ...
  public void OnItemSelected(FileBrowserItem item) { SelectedFile = item; }
  ...
  public char OnValidateFilenameInput(string text, int charIndex,
                                      char addedChar) {
    if (addedChar == '\n') {
      OnSubmitButtonClicked();
      return '\0';
...
#region Helper Functions
      ...
      try {
        ...
        if ((item.Attributes & FileAttributes.Directory) == 0) {
          ...
          HashSet<string> extensions =
              filters[filtersDropdown.value].extensions;
        ...
        if (m_searchString.Length == 0 ||
            item.Name.ToLower().Contains(searchStringLowercase))
          validItems.Add(item);
      } catch (Exception e) {
  ...
  private bool AddQuickLink(Sprite icon, string name, string path,
    ...
    FileBrowserQuickLink quickLink = (FileBrowserQuickLink)Instantiate(
        quickLinkPrefab, quickLinksContainer, false);
  ...
  private string GetPathWithoutTrailingDirectorySeparator(string path) {
    ...
    // Credit:
    // http://stackoverflow.com/questions/6019227/remove-the-last-character-if-its-directoryseparatorchar-with-c-sharp
    try {
  ...
  // Credit:
  // http://answers.unity3d.com/questions/898770/how-to-get-the-width-of-ui-text-with-horizontal-ov.html
  private int CalculateLengthOfDropdownText(string str) {
    ...
    myFont.RequestCharactersInTexture(str, filterItemTemplate.fontSize,
                                      filterItemTemplate.fontStyle);

    for (int i = 0; i < str.Length; i++) {
      if (!myFont.GetCharacterInfo(str[i], out characterInfo,
                                   filterItemTemplate.fontSize))
        totalLength += 5;
...
#region File Browser Functions(static)
  ...
  public static IEnumerator WaitForSaveDialog(bool folderMode = false,
                                              string initialPath = null,
                                              string title = "Save",
                                              string saveButtonText = "Save") {
    if (!ShowSaveDialog(null, null, folderMode, initialPath, title,
                        saveButtonText))
      yield break;
  ...
  WaitForLoadDialog(bool folderMode = false, string initialPath = null,
                    string title = "Load", string loadButtonText = "Select") {
    if (!ShowLoadDialog(null, null, folderMode, initialPath, title,
                        loadButtonText))
      yield break;
  ...
  public static bool AddQuickLink(string name, string path,
                                  Sprite icon = null) {
    Vector2 anchoredPos =
        new Vector2(0f, -Instance.quickLinksContainer.sizeDelta.y);
  ...
  public static void SetExcludedExtensions(params string[] excludedExtensions) {
    ...
    if (excludedExtensions != null) {
      for (int i = 0; i < excludedExtensions.Length; i++)
        Instance.excludedExtensionsSet.Add(
            excludedExtensions[i].ToLowerInvariant());
    }
  }
  ...
  private static void SetFiltersPostProcessing() {
    ...
    for (int i = 0; i < filters.Count; i++) {
      ...
      maxFilterStrLength =
          Mathf.Max(maxFilterStrLength,
                    Instance.CalculateLengthOfDropdownText(filterStr));
    }
...
#if !UNITY_EDITOR && UNITY_ANDROID
    Permission result =
        (Permission)AJC.CallStatic<int>("CheckPermission", Context);
    if (result == Permission.Denied &&
        (Permission)PlayerPrefs.GetInt("FileBrowserPermission",
                                       (int)Permission.ShouldAsk) ==
            Permission.ShouldAsk)
  ...
  public static Permission RequestPermission() {
    ...
    lock (threadLock) {
      FBPermissionCallbackAndroid nativeCallback =
          new FBPermissionCallbackAndroid(threadLock);

      AJC.CallStatic("RequestPermission", Context, nativeCallback,
                     PlayerPrefs.GetInt("FileBrowserPermission",
                                        (int)Permission.ShouldAsk));

      if (nativeCallback.Result == -1)
        System.Threading.Monitor.Wait(threadLock);

      if ((Permission)nativeCallback.Result != Permission.ShouldAsk &&
          PlayerPrefs.GetInt("FileBrowserPermission", -1) !=
