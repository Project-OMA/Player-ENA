...
public class ObjectivesController : MonoBehaviour {
  ...
  void Update() {
    ...
    if (Input.GetButtonDown("Jump")) {
      ...
      if (stageCount < 3) {
        EasyTTSUtil.SpeechAdd("Seu objetivo é encontrar " +
                              ((Objective)allObjectives[stageCount]).nameTTS);
      } else {
    ...
    // stopAllAudios();
    // ObjectivesController scrip =
    // (ObjectivesController)GetComponentInParent(typeof(ObjectivesController));
    ...
    if (stageCount == 4 && ((Objective)allObjectives[3]).hitSound.isPlaying ==
    false)
    {
    ...
    if (col.gameObject.tag != "carpet") {
      ...
      if (col.gameObject.name == "Doors" && stageCount == 3) {
        ...
        StartCoroutine(EndGame());
               ...
               ((Objective)allObjectives[stageCount]).name) {
        audiosStarted = true;
        ...
        EasyTTSUtil.SpeechAdd("Você encontrou " +
                              ((Objective)allObjectives[stageCount]).nameTTS);
        if (!enableAllObjectives)
        ...
        if (stageCount < 3) {
          EasyTTSUtil.SpeechAdd("Agora você deve encontrar " +
                                ((Objective)allObjectives[stageCount]).nameTTS);
          if (!enableAllObjectives)
  ...
  public void playAudioRescue() { audioRescue.Play(); }

  public AudioSource getAudioRescue() { return audioRescue; }
  ...
  public void selectObjectives() {
    ...
    if (enableAllObjectives)
      allObjectives.RemoveRange(numberObjectives,
                                allObjectives.Count - numberObjectives);
  ...
  public string GetObjectivesNames() {
    string names = ((Objective)allObjectives[0]).nameTTS + ", " +
                   ((Objective)allObjectives[1]).nameTTS + " e " +
                   ((Objective)allObjectives[2]).nameTTS;
  ...
  public void saveUserStatus() {
    string path = Application.persistentDataPath + '/' + UserModel.username +
                  "_" + sceneName + "_timeLog.txt";
  ...
  private IEnumerator shoot() {
    ...
    // totalTime.text = "tempo total:" + Action.gameTime.ToString("F0") + "
    // segundos."; ScreenCapture.CaptureScreenshot(UserModel.username + "/" +
    // sceneName + "_Tracker" + ".png");
    System.DateTime currentTime = System.DateTime.Now;

    string time =
        currentTime.Hour + "_" + currentTime.Minute + "_" + currentTime.Second;

    ScreenCapture.CaptureScreenshot(UserModel.username + "_" + sceneName + "_" +
                                    time + "_Tracker" + ".png");
  ...
  private void OnApplicationQuit() { print("Fechou"); }
  ...
  public void EndTest() { StartCoroutine(QuitGame()); }
